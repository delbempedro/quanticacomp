!------------------------------------------------------------
! File: P2-5255417-ex-2.f90
!
! Description:
!   Determining the ground-state energy (E0) of an infinite modified potential well by finding the eigenvalue of the discretized second-derivative matrix (in the limit n->infinity) via the power method.
!
! Dependencies:
!   - None
!
...
!------------------------------------------------------------
program InfiniteModifiedPotentialWell
    ...
    !open results file
    open(unit=1, file="P2-5255417-ex-2-results.txt", status="replace")
...
contains
    ...
    subroutine solve_Aw(v, w, matrix_dimension, A_main_diagonal, A_other_diagonals)
        ...
        real(kind=dp) :: denominator, v_line(matrix_dimension), A_other_diagonals_line(matrix_dimension - 1), modified_first_element

        modified_first_element = -1.0_dp

        if (matrix_dimension == 1) then
            w(1) = v(1) / modified_first_element
            return
        else

            !forward step
            A_other_diagonals_line(1) = A_other_diagonals / modified_first_element
            v_line(1) = v(1) / modified_first_element
...
end program InfiniteModifiedPotentialWell